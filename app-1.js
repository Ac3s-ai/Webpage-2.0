...
let bounceStarted = false; // Flag to control bounce execution
...
function runCarousel() {
    ...
    // Add bounce after the transition completes
    if (!bounceStarted) {
      setTimeout(() => {
        list.classList.add('bounce');
        bounceStarted = true;
        setTimeout(() => list.classList.remove('bounce'), 1000); //remove bounce class after animation
      }, 500); // Adjust delay as needed to match transition duration
      
    } else {
      bounceStarted = false; // Reset for next transition
    }
}
runCarousel();
